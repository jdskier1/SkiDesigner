...
# This software is provided as-is, without warranty. Users are responsible for validating generated ski, mold, and CNC outputs before manufacturing.
# Written by JD Ritchey from Red Barrel Skis
...
from PySide6.QtCore import Qt, QRectF, QPointF, QSize, QDateTime, QMarginsF
import json
...
def get_default_shape_presets():
    return {
        ...
        "Carving Ski": {
            "edge": [
                {"pos": (0, 750), "in": (-58, 750), "out": (30, 750)},
                {"pos": (66, 560), "in": (92, 700), "out": (48, 340)},
                {"pos": (32, -90), "in": (32, 80), "out": (32, -260)},
                {"pos": (74, -780), "in": (48, -560), "out": (104, -940)},
                {"pos": (0, -1000), "in": (30, -1000), "out": (-65, -1000)},
            ],
            "core": [
                {"pos": (-750, 750), "in": (-750, 800), "out": (-750, 700)},
                {"pos": (-750, 560), "in": (-750, 620), "out": (-750, 470)},
                {"pos": (-725, 180), "in": (-725, 260), "out": (-725, 60)},
                {"pos": (-725, -420), "in": (-725, -320), "out": (-725, -560)},
                {"pos": (-750, -800), "in": (-750, -700), "out": (-750, -920)},
                {"pos": (-750, -1000), "in": (-750, -940), "out": (-750, -1060)},
            ],
            "camber": [
                {"pos": (-400, 800),  "in": (-410, 800), "out": (-400, 790)},
                {"pos": (-400, 750),  "in": (-400, 760), "out": (-450, 700)},
                {"pos": (-452, 560),  "in": (-452, 595), "out": (-452, 515)},
                {"pos": (-425, -90),  "in": (-425, 300), "out": (-425, -480)},
                {"pos": (-452, -780), "in": (-452, -740), "out": (-452, -820)},
                {"pos": (-400, -1000),"in": (-450, -950), "out": (-400, -1010)},
                {"pos": (-400, -1050),"in": (-400, -1040), "out": (-410, -1050)},
            ],
        },
...
class SkiShape(QGraphicsItem):
    ...
    def _iter_regular_point_groups(self):
        return (self.points, self.core_thickness_points, self.camber_thickness_points)
    ...
    def apply_design_constraints(self):
        ...
        # This one aligns core thickness with length of ski(not inlcuding dovetail) 
    ...
    def apply_startup_values(self, values):
        ...
        self.prepareGeometryChange()
        self.update()
    def get_core_trim_boundaries_y(self):
        tail_y = self.points[0].pos.y()
        tip_y = self.points[-1].pos.y()
    ...
    def build_full_shape(self):
        ...
        full.connectPath(self.build_mirrored_edge_path())
        full.closeSubpath()
    ...
    def get_leftmost_x(self):
        ...
        # mirrored side is negative X
        leftmost_x = min(-p.x() for p in pts)
    ...
    def get_outline_points_cm1(self, samples=1000):
        ...
        mirrored = self.build_mirrored_edge_path()
    ...
    def path_to_points_cm(self, path):
        shapes = []
        for poly in path.toSubpathPolygons():
            pts = [(-pt.y() / PIXELS_PER_CM, pt.x() / PIXELS_PER_CM) for pt in poly]
            if len(pts) >= 2 and pts[0] == pts[-1]:
                pts = pts[:-1]
            if pts:
                shapes.append(pts)
        return shapes
    def path_to_fill_points_cm(self, path):
    ...
    def _build_strip_piece_paths(self, start_y, end_y, outer_inset_px, inner_inset_px, samples=2000):
        ...
        outer_right = self._half_outline_between(samples_sorted, start_y, end_y, inset_px=outer_inset_px)
        inner_right = self._half_outline_between(samples_sorted, start_y, end_y, inset_px=inner_inset_px)
        if len(outer_right) < 2 or len(inner_right) < 2:
            return []

        right_polygon = outer_right + list(reversed(inner_right))
        left_outer = [QPointF(-p.x(), p.y()) for p in outer_right]
        left_inner = [QPointF(-p.x(), p.y()) for p in inner_right]
        left_polygon = left_outer + list(reversed(left_inner))
    ...
    def get_export_shapes(self, samples=5000, include_all=True):
        ...
        if True if include_all else self.show_sidewall_shape:
            ...
            inner_inset = self.edge_thickness_px + self.sidewall_thickness_px
    ...
    def draw_sidecut_circle(self, painter):
        ...
        # Use control points 2, 3, 4 directly
        A = self.points[1].pos
        B = self.points[2].pos
        C = self.points[3].pos
        ...
        if abs(d) < 0.001:
            return  # Points are nearly collinear
        ...
        ski_true_center_y = (self.points[0].pos.y() + self.points[-1].pos.y()) / 2
        ski_true_center_x = 0
        ski_turn_center_y = self.points[2].pos.y()
        ski_turn_center_x = self.points[2].pos.x()
        ski_true_center = QPointF(ski_true_center_x, ski_true_center_y)
        ski_turn_center = QPointF(ski_turn_center_x, ski_turn_center_y)
...
#            angle_deg = math.degrees(math.acos(cosang))          
        ...
        mid = QPointF(B.x() + 20, B.y()+15)
        painter.setPen(QPen(QColor(255,255,255), 1))
        painter.drawText(mid, f"{angle_deg:.2f}°")
    ...
    def _seam_local_to_world(self, seam_name, pt):
        ...
        return QPointF(pt.x(), data["y"] + data["direction"] * pt.y())
    def _seam_world_to_local(self, seam_name, pt):
        data = self._get_seam_editor_data(seam_name)
        return QPointF(pt.x(), data["direction"] * (pt.y() - data["y"]))
    def _sample_cubic_chain(self, seam_points, end_x, center_point=None, samples_per_seg=18):
        ...
        if not seam_points:
            ...
            edge_in = QPointF(end_x - max(10.0, abs(end_x) * 0.22), 0.0)
            total_samples = max(24, int(samples_per_seg))
            path.cubicTo(start_out, edge_in, QPointF(end_x, 0.0))
            return [path.pointAtPercent(i / total_samples) for i in range(total_samples + 1)]
        first = seam_points[0]
        ...
        last = seam_points[-1]
        edge_in = QPointF(end_x - max(10.0, abs(end_x) * 0.22), 0.0)
        path.cubicTo(last.handle_out, edge_in, QPointF(end_x, 0.0))
        pts = []
    ...
    def _sample_puzzle_seam(self, y, half_width, direction, seam_name=None, samples_per_seg=18):
        ...
        if seam_count <= 0:
            return [QPointF(-half_width, y), QPointF(half_width, y)]
        center_src = self.tip_seam_center if seam_name == "tip" else self.tail_seam_center
        center_point = SeamControlPoint(
            QPointF(0.0, direction * center_src.pos.y()),
            QPointF(center_src.handle_in.x(), direction * center_src.handle_in.y()),
            QPointF(center_src.handle_out.x(), direction * center_src.handle_out.y()),
        )
        right_points = []
        for p in self._active_seam_points(seam_name):
            right_points.append(
                SeamControlPoint(
                    QPointF(p.pos.x(), direction * p.pos.y()),
                    QPointF(p.handle_in.x(), direction * p.handle_in.y()),
                    QPointF(p.handle_out.x(), direction * p.handle_out.y()),
                )
            )
        right = self._sample_cubic_chain(right_points, half_width, center_point=center_point, samples_per_seg=samples_per_seg)
        left = [QPointF(-p.x(), p.y()) for p in reversed(right[1:])]
        return [QPointF(p.x(), p.y() + y) for p in (left + right)]
    ...
    def _set_seam_handle_from_pos(self, seam_name, kind, index, pos):
        ...
        if self.owner_window is not None:
            self.owner_window.sync_seam_editor_widgets()
        self.update()
    def _build_region_path(self, y_start, y_end, inset_px=0.0, top_seam=False, top_direction=-1, bottom_seam=False, bottom_direction=1, samples=2000):
        if y_end < y_start:
        ...
        samples_sorted = self._sorted_edge_samples(samples)
        right = self._half_outline_between(samples_sorted, y_start, y_end, inset_px=inset_px)
        if len(right) < 2:
            return QPainterPath()

        top_hw = right[0].x()
        bot_hw = right[-1].x()
        top = self._sample_puzzle_seam(y_start, top_hw, top_direction, "tip" if top_direction < 0 else "tail") if top_seam else [QPointF(-top_hw, y_start), QPointF(top_hw, y_start)]
        bottom_lr = self._sample_puzzle_seam(y_end, bot_hw, bottom_direction, "tip" if bottom_direction < 0 else "tail") if bottom_seam else [QPointF(-bot_hw, y_end), QPointF(bot_hw, y_end)]
        bottom = list(reversed(bottom_lr))
        left = [QPointF(-p.x(), p.y()) for p in reversed(right)]

        polygon = list(top)
        polygon.extend(right[1:])
        polygon.extend(bottom[1:])
        polygon.extend(left[1:-1])
        return self._polygon_path(polygon)
    def _build_spacer_region_path(self, y_start, y_end, seam_at_top=False, seam_direction=1, seam_inset_px=0.0, samples=2000):
        if y_end < y_start:
            y_start, y_end = y_end, y_start
            seam_at_top = not seam_at_top

        samples_sorted = self._sorted_edge_samples(samples)
        outer_right = self._half_outline_between(samples_sorted, y_start, y_end, inset_px=0.0)
        if len(outer_right) < 2:
            return QPainterPath()

        eps = 1e-6
        if seam_at_top:
            seam_y = y_start
            seam_outer_hw = outer_right[0].x()
            seam_inner_hw = self._half_width_at_y(samples_sorted, seam_y, inset_px=seam_inset_px)
            seam_lr = self._sample_puzzle_seam(seam_y, seam_inner_hw, seam_direction)
            tail_hw = outer_right[-1].x()
            polygon = [QPointF(-seam_outer_hw, seam_y)]
            if seam_outer_hw > seam_inner_hw + eps:
                polygon.append(QPointF(-seam_inner_hw, seam_y))
            polygon.extend(seam_lr[1:])
            if seam_outer_hw > seam_inner_hw + eps:
                polygon.append(QPointF(seam_outer_hw, seam_y))
            polygon.extend(outer_right[1:])
            polygon.append(QPointF(-tail_hw, y_end))
            polygon.extend(QPointF(-p.x(), p.y()) for p in reversed(outer_right[1:-1]))
            return self._polygon_path(polygon)
        else:
            seam_y = y_end
            tip_hw = outer_right[0].x()
            seam_outer_hw = outer_right[-1].x()
            seam_inner_hw = self._half_width_at_y(samples_sorted, seam_y, inset_px=seam_inset_px)
            seam_rl = list(reversed(self._sample_puzzle_seam(seam_y, seam_inner_hw, seam_direction)))
            polygon = [QPointF(-tip_hw, y_start), QPointF(tip_hw, y_start)]
            polygon.extend(outer_right[1:])
            if seam_outer_hw > seam_inner_hw + eps:
                polygon.append(QPointF(seam_inner_hw, seam_y))
            polygon.extend(seam_rl[1:])
            if seam_outer_hw > seam_inner_hw + eps:
                polygon.append(QPointF(-seam_outer_hw, seam_y))
            polygon.extend(QPointF(-p.x(), p.y()) for p in reversed(outer_right[1:-1]))
            return self._polygon_path(polygon)
    ...
    def build_base_material_path(self, samples=2000):
        ...
        if key not in self._cache:
            full_shape = self._shape_path_from_samples(self.sample_edge(samples), inset_px=0.0)
            edge_path = self.build_edge_inlay_path(samples)
            self._cache[key] = full_shape.subtracted(edge_path)
        return self._cache[key]
    ...
    def build_edge_inlay_path(self, samples=2000):
        ...
        if key not in self._cache:
            outer = self._shape_path_from_samples(
                self.sample_edge(samples),
                inset_px=0,
                tip_trim_px=self.edge_inlay_tip_trim_px,
                tail_trim_px=self.edge_inlay_tail_trim_px,
            )
            inner = self._shape_path_from_samples(
                self.sample_edge(samples),
    ...
    def build_sidewall_path(self, samples=2000):
        ...
        if key not in self._cache:
            outer = self._shape_path_from_samples(
                self.sample_edge(samples),
                inset_px=0,
                tip_trim_px=self.tip_spacer_length_px,
                tail_trim_px=self.tail_spacer_length_px,
            )
            inner = self._shape_path_from_samples(
                self.sample_edge(samples),
                inset_px=self.edge_thickness_px + self.sidewall_thickness_px,
    ...
    def build_tip_spacer_path(self, samples=2000):
        ...
        if key not in self._cache:
            tip_y, tail_y, core_tip_start_y, core_tail_end_y = self.get_core_trim_boundaries_y()
            seam_inset_px = self.edge_thickness_px + self.sidewall_thickness_px
            self._cache[key] = self._build_spacer_region_path(
                tip_y,
                core_tip_start_y,
                seam_at_top=False,
                seam_direction=-1,
                seam_inset_px=seam_inset_px,
                samples=samples,
            )
        return self._cache[key]
    ...
    def build_sidewall_spacer_shell_path(self, samples=2000):
        ...
        if key not in self._cache:
            outer = self._shape_path_from_samples(self.sample_edge(samples), inset_px=0.0)
            core = self.build_core_footprint_path(samples)
            shell = outer.subtracted(core)

            # Keep the shell tied to the existing sidewall + spacer geometry so
            # it matches the visible layup pieces and seam transitions.
            shell = shell.united(self.build_sidewall_path(samples))
            shell = shell.united(self.build_tip_spacer_path(samples))
            shell = shell.united(self.build_tail_spacer_path(samples))
            self._cache[key] = shell.simplified()
        return self._cache[key]
    ...
    def get_full_outline_polygon_cm(self, samples=720):
        ...
        if key not in self._cache:
            ...
            left_path = self.build_mirrored_edge_path()
    ...
    def _append_explicit_seam_stitch(self, faces, seam_spans, body_ring, seam_is_start):
        """Create a dedicated seam-plane stitch between a multi-span dovetail
        cross-section and the single-span body envelope at the exact same x.

        This fills the gaps between adjacent dovetail spans, instead of relying
        on looser post-mesh hole repair passes.
        """
        ...
        for i in range(len(ordered) - 1):
            ...
            self._append_station_cap(faces, gap_ring, at_start=seam_is_start)

    def _dedupe_and_drop_degenerate_faces(self, faces, area_tol=1e-7):
        if not faces:
            return faces
    ...
    def _match_station_spans(self, prev_spans, curr_spans):
        ...
        candidates = []
        for i, (py0, py1, _) in enumerate(prev_spans):
            pc = 0.5 * (py0 + py1)
            pw = py1 - py0
            for j, (cy0, cy1, _) in enumerate(curr_spans):
                cc = 0.5 * (cy0 + cy1)
                cw = cy1 - cy0
        ...
        for _neg_overlap, _norm_dist, center_dist, i, j in sorted(candidates):
            if i in used_prev or j in used_curr:
                continue
            py0, py1, _ = prev_spans[i]
            cy0, cy1, _ = curr_spans[j]
            overlap = min(py1, cy1) - max(py0, cy0)
    ...
    def _scan_outline_stations(self, station_count=360, outline_samples=1200):
        ...
        stations = [
            min_x + (max_x - min_x) * i / max(1, station_count - 1)
    ...
    def _find_dovetail_zone_end_from_start(self, stations, spans_by_station, min_run=2):
    ...
    def _find_dovetail_zone_start_from_end(self, stations, spans_by_station, min_run=2):
        multi = [i for i, spans in enumerate(spans_by_station) if len(spans) > 1]
        if not multi:
            return None
        run_end = multi[-1]
    ...
    def _build_standard_mesh_range(self, x_start_cm, x_end_cm, num_samples=500, cap_start=True, cap_end=True, leading_ring=None, trailing_ring=None):
        ...
        faces = []
        outline = self.get_half_outline_points_cm(max(400, num_samples * 2))
        if not outline:
            return []
        ...
        for i in range(num_samples + 1):
            ...
            width_cm = self.get_width_at_length(length_cm, outline)
            camber_cm = self.get_camber_at_length(length_cm)
            thickness_cm = self.get_thickness_at_length(length_cm)
            half_w = width_cm / 2.0
            BL = (length_cm, -half_w, camber_cm)
            BR = (length_cm,  half_w, camber_cm)
            TL = (length_cm, -half_w, camber_cm + thickness_cm)
            TR = (length_cm,  half_w, camber_cm + thickness_cm)
            sections.append((TL, TR, BL, BR))
    ...
    def _build_dovetail_patch_range(self, outline, x_start_cm, x_end_cm, num_samples=220, cap_start=True, cap_end=True, return_seam_rings=False):
        ...
        for i in range(span_count + 1):
            ...
            curr_spans = [
                (y0, y1, self._build_span_ring_cm(x_station, y0, y1))
            ...
            if not curr_spans:
                if prev_spans:
                    for _, _, ring in prev_spans:
                        self._append_station_cap(faces, ring, at_start=False)
                    prev_spans = None
                continue

            if prev_spans is None:
                if cap_start and i == 0:
                    for _, _, ring in curr_spans:
                        self._append_station_cap(faces, ring, at_start=True)
            ...
            for pi, cj in matches:
                self._append_bridge_faces(faces, prev_spans[pi][2], curr_spans[cj][2])
    ...
    def build_ski_mesh_dovetail(self, num_samples=260):
        ...
        # Dedicated explicit seam stitch: fill the exact seam-plane gaps between
        # the dovetail spans and the one-ring body envelope, instead of relying
        # on progressively looser post-mesh hole repair passes.
        ...
        faces = self._dedupe_and_drop_degenerate_faces(faces)

        self._cache[key] = faces
        return faces

    def build_ski_mesh(self, num_samples=120):
        requested_samples = int(num_samples)
        if getattr(self, "interaction_3d_active", False) and getattr(self, "low_res_3d_edit_mode", True):
        ...
        key = self._cache_key("ski_mesh_dispatch", requested_samples)
        if key in self._cache:
            return self._cache[key]
    ...
    def get_projected_3d_model_bounds(self, samples=80):
        ...
        projected = []
        seen = set()
        duplicate_shift_px = -float(getattr(self, "second_3d_offset_px", 165.0)) if self.show_ski_snowboard else None
            ...
            for vertex in face:
                ...
                if duplicate_shift_px is not None:
                    if base_to_base:
                        ...
                        projected.append(QPointF(pt.x() + duplicate_shift_px, pt.y()))
    ...
    def _get_3d_graphic_bounds_cm(self):
        outline = self.get_half_outline_points_cm(900)
        if not outline:
            return (-100.0, 100.0, -10.0, 10.0)
        xs = [float(p[0]) for p in outline]
        half_width = max((abs(float(p[1])) for p in outline), default=10.0)
        half_width = max(half_width, 1.0)
        return (min(xs), max(xs), -half_width, half_width)
    ...
    def _draw_3d_graphic_mapped_face(self, painter, face, pts, image, face_kind, bounds_cm=None):
        if image is None or image.isNull() or len(face) < 3 or len(pts) < 3:
            return
        if bounds_cm is None:
            bounds_cm = self._get_3d_graphic_bounds_cm()

        src_pts = [self._face_uv_to_image_point(v, image, face_kind, bounds_cm) for v in face]
    ...
    def _draw_3d_graphic_overlay(self, painter, draw_faces, face_kind, image):
        if image is None or image.isNull() or not draw_faces:
            return
        bounds_cm = self._get_3d_graphic_bounds_cm()
        for _depth, face, _rotated, pts in draw_faces:
            kind = self._classify_3d_surface_face(face)
            if kind != face_kind or len(face) < 3 or len(pts) < 3:
                continue
            self._draw_3d_graphic_mapped_face(painter, face, pts, image, face_kind, bounds_cm=bounds_cm)
    ...
    def _segment_hidden_by_faces(self, mid_rotated, mid_projected, draw_faces, depth_bias=1e-4):
        segment_depth = self._point_depth(mid_rotated)
        test_pt = QPointF(mid_projected.x(), mid_projected.y())
        for face_depth, _face, _rotated, pts in draw_faces:
            poly = QPolygonF(pts)
            if not poly.boundingRect().adjusted(-0.5, -0.5, 0.5, 0.5).contains(test_pt):
                continue
    ...
    def _draw_visible_segment(self, painter, a_rot, a_pt, b_rot, b_pt, draw_faces, samples=10):
        visible_run = []
        for i in range(samples + 1):
            t = i / float(samples)
            rot = (
            ...
            hidden = self._segment_hidden_by_faces(rot, pt, draw_faces)
            if not hidden:
    ...
    def _draw_3d_outline_edges(self, painter, draw_faces, duplicate_shift_px=None, samples=240):
        ...
        for depth, _face, rotated, pts in draw_faces:
            ...
            if not keep and len(entry['normals']) >= 2:
                ...
                for n1 in entry['normals'][1:]:
                    ...
                    if dot < crease_dot_threshold:
                        ...
                        break
            if keep:
                painter.drawLine(entry['a'], entry['b'])
    ...
    def paint(self, painter, option, widget):
        ...
        if self.show_sidewall_spacer_shell:
            ...
            painter.drawPath(self.build_sidewall_spacer_shell_path())
        ...
        if self.show_3d:
            ...
            for raw_face in faces:
                ...
                if not self._face_visible(rotated):
                    continue
                ...
                if duplicate_shift_px is not None:
                    if base_to_base:
                        ...
                        if self._face_visible(rotated2):
                            ...
                            draw_faces.append((depth2, face2, rotated2, pts2))
                    else:
                        pts2 = [QPointF(pt.x() + duplicate_shift_px, pt.y()) for pt in pts]
                        draw_faces.append((depth, face, rotated, pts2))
            ...
            if render_mode == "wireframe":
                ...
                for _depth, _face, _rotated, pts in draw_faces:
                    painter.drawPolygon(QPolygonF(pts))
            else:
                painter.setPen(Qt.PenStyle.NoPen)
                for depth, face, rotated, pts in draw_faces:
                    poly = QPolygonF(pts)
            ...
            if render_mode in {"shaded_edges", "wireframe"}:
                self._draw_3d_outline_edges(painter, draw_faces, duplicate_shift_px=duplicate_shift_px)
        # Ski/Snowboard
        if self.show_ski_snowboard:
            painter.save()
            leftmost_x = self.get_leftmost_x()
            painter.translate(2 * leftmost_x - 10, 0)

            second_ski_path = self.build_full_shape()
        ...
        if owner is None or not hasattr(owner, "should_draw_stats") or owner.should_draw_stats():
            ...
            stats = [                               #All the dynamic stats
                ...
                f"Turning Radius: {.01*self.get_turning_radius():.1f} m",
    ...
    def _draw_relative_point_lock_cues(self, painter):
        ...
        for entry in self.relative_point_locks:
            ...
            if entry["axis"] == "vertical":
                ...
                painter.drawText(QPointF(x + 8.0, (y0 + y1) / 2.0 - 4.0), "V")
            elif entry["axis"] == "horizontal":
                ...
                painter.drawText(QPointF((x0 + x1) / 2.0 + 6.0, y - 8.0), "H")
        painter.restore()
    def _draw_point_circles(self, painter):
        ...
        painter.save()
        painter.setBrush(Qt.BrushStyle.NoBrush)
        for pts, center, radius in entries:
    ...
    def _draw_point_dimensions(self, painter):
        ...
        if not entries:
            return
        painter.save()
        for point_a, point_b, distance_px in entries:
    ...
    def keyPressEvent(self,event):
        ...
        if event.modifiers() == Qt.KeyboardModifier.ControlModifier:
            ...
            selected_points = self._get_selected_circle_points()
            if len(selected_points) == 3:
                if self._circle_from_points(selected_points[0].pos, selected_points[1].pos, selected_points[2].pos) is not None:
                    self.push_undo()
...
class StartupDialog(QDialog):
    def __init__(self, initial_values, parent=None):
        ...
        title_label2.setStyleSheet("font-family: Kristen ITC; font-size: 20px; font-weight: bold;")
        ...
        self.load_file_button = QPushButton("Load Ski File")
        self.load_file_button.setMinimumHeight(40)
        self.load_file_button.clicked.connect(self.load_startup_file)
        layout.addWidget(self.load_file_button)


        title_label = QLabel("New Design")
        title_label.setStyleSheet("font-family: Kristen ITC; font-size: 20px; font-weight: bold;")
...
class CNCToolpath3DView(QWidget):
    ...
    def paintEvent(self, event):
        ...
        info_font.setPointSize(max(info_font.pointSize() + 22, 11))
        stats_font = QFont(base_font)
        stats_font.setPointSize(max(stats_font.pointSize() + 2, 11))
        ...
        painter.drawText(title_rect, int(Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignRight), "RMB orbit  •  Shift changes left/right from pitch to roll  •  MMB pan  •  Wheel zoom")

        viewport_rect = rect.adjusted(10, 38, -10, -12)
        self._ensure_geometry_cache()
        ...
        self._draw_coordinate_axes_overlay(painter, viewport_rect, projected, label_font=stats_font)

        painter.setPen(QColor(195, 200, 208))
        painter.setFont(stats_font)
        wx, wy, wz = self._workspace_size
        ox, oy, oz = self._workspace_origin
        stats_rect = viewport_rect.adjusted(8, 8, -8, -8)
        preview_x, preview_y = self._preview_offset_xy
        painter.drawText(stats_rect, int(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignBottom),
                         f"Workspace: X {wx:.0f}  Y {wy:.0f}  Z {wz:.0f} mm")
        painter.drawText(stats_rect.adjusted(0, 0, 0, -24), int(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignBottom),
                         f"Preview offset: X {preview_x:.0f}  Y {preview_y:.0f} mm")
        painter.drawText(stats_rect.adjusted(0, 0, 0, -48), int(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignBottom),
                         f"Workspace move: X {ox:.0f}  Y {oy:.0f}  Z {oz:.0f} mm")
        painter.drawText(stats_rect, int(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignBottom),
...
class MainWindow(QWidget):
    def __init__(self, startup_values=None):
        ...
        for spin in [self.edge_thickness_spin, self.sidewall_thickness_spin, self.edge_inlay_tip_spin, self.edge_inlay_tail_spin, self.tip_spacer_spin, self.tail_spacer_spin, self.seam_depth_spin]:
        ...
        layup_scroll_boxes_layout.addWidget(QLabel("Sidewall Thickness"), 0, 0)
        layup_scroll_boxes_layout.addWidget(self.sidewall_thickness_spin, 0, 1)
        layup_scroll_boxes_layout.addWidget(QLabel("Edge Thickness"), 1, 0)
        layup_scroll_boxes_layout.addWidget(self.edge_thickness_spin, 1, 1)
        layup_scroll_boxes_layout.addWidget(QLabel("Edge to Tip"), 2, 0)
        layup_scroll_boxes_layout.addWidget(self.edge_inlay_tip_spin, 2, 1)
        layup_scroll_boxes_layout.addWidget(QLabel("Edge to Tail"), 3, 0)
        layup_scroll_boxes_layout.addWidget(self.edge_inlay_tail_spin, 3, 1)
        layup_scroll_boxes_layout.addWidget(QLabel("Minimum Core Thickness"), 4, 0)
        layup_scroll_boxes_layout.addWidget(self.minimum_core_thickness_spin, 4, 1)
        layup_scroll_boxes_layout.addWidget(QLabel("Tip Spacer Length"), 5, 0)
        layup_scroll_boxes_layout.addWidget(self.tip_spacer_spin, 5, 1)
        layup_scroll_boxes_layout.addWidget(QLabel("Tail Spacer Length"), 6, 0)
        layup_scroll_boxes_layout.addWidget(self.tail_spacer_spin, 6, 1)
        layup_scroll_boxes_layout.addWidget(QLabel("Tip Seam Pts"), 7, 0)
        layup_scroll_boxes_layout.addWidget(self.tip_seam_point_count_spin, 7, 1)
        layup_scroll_boxes_layout.addWidget(QLabel("Tail Seam Pts"), 8, 0)
        layup_scroll_boxes_layout.addWidget(self.tail_seam_point_count_spin, 8, 1)
...
#            self.view.viewport().update()
    ...
    def rescale_cnc_preview_view(self):
        preview_widget = getattr(self, 'cnc_3d_preview_widget', None)
        if preview_widget is not None:
            preview_widget.fit_toolpath_to_view(0.90, self._active_cnc_preview_fit_rect())
        if hasattr(self, "ski") and self.ski is not None:
    ...
    def toggle_ski_snowboard(self):
        ...
        if hasattr(self, "toggle_ski_snowboard_action"):
            ...
            self.toggle_ski_snowboard_action.setText("Show Single Ski" if self.ski.show_ski_snowboard else "Show 2nd Ski")
    ...
    def _sync_second_ski_layout_button(self):
        ...
        if hasattr(self, "toggle_second_ski_button"):
            self.toggle_second_ski_button.setText("Show Single Ski" if show_second else "Show 2nd Ski")
        if hasattr(self, "second_ski_spacing_slider"):
    ...
    def _friendly_cnc_shape_name(self, shape_name):
        if str(shape_name) == "camber_profile":
            return "Lower Mold/Camber"
        if str(shape_name) == "core_footprint":
            return "Core"
        if str(shape_name) == "sidewall_spacer_shell":
            return "Shell"
        return str(shape_name).replace("_", " ").title()
    ...
    def _selected_cnc_shape(self):
        for shape in self._get_cnc_shapes():
            if shape.get("name") == self.selected_cnc_shape_name:
                return shape
        return None
    ...
    def refresh_cnc_shape_buttons(self):
        ...
        for idx, name in enumerate(unique_names):
            ...
            self.cnc_shape_grid.addWidget(btn, idx // 2, idx % 2)
    ...
    def _loops_mm_from_qpath(self, qpath):
        loops_mm = []
        try:
            loops_cm = self.ski.path_to_points_cm(qpath)
        except Exception:
            loops_cm = []
    ...
    def _shape_paths_mm_for_operation(self, shape):
        ...
        cache_key = ("shape_paths", self._cnc_shape_cache_key(shape), op, round(radius, 4))
        ...
        for loop_index, loop in enumerate(base_loops):
            ...
            if multi_loop and loop_index > 0 and offset_sign != 0.0:
                offset_sign *= -1.0

            if offset_sign != 0.0:
                result = self._offset_closed_polygon(loop, radius * offset_sign)
                result = self._clean_offset_loop_mm(result, radius)
                result = self._remove_near_collinear_points_mm(self._normalized_path_points_mm(result))
            else:
    ...
    def _pocket_paths_mm(self, shape):
        tool_diameter = max(0.1, float(self.cnc_tool_diameter_spin.value()))
        cache_key = ("pocket", self._cnc_shape_cache_key(shape), round(tool_diameter, 4))
        cached = self._cnc_toolpath_cache.get(cache_key)
        if cached is not None:
            return self._clone_path_loops_mm(cached)
        ...
        for loop_index, loop in enumerate(base_loops):
            offset_sign = -1.0
            if multi_loop and loop_index > 0:
                offset_sign = 1.0
    ...
    def build_cnc_gcode(self):
        ...
        if selected is None:
            if shapes:
                selected = shapes[0]
                self.selected_cnc_shape_name = selected.get("name")
            else:
    ...
    def _draw_cnc_projection_panel(self, painter, view_rect, view_name, segment_groups, projector, axis_h, axis_v, mill_pen, plunge_pen, ramp_pen, rapid_pen, panel_bg, border_color, axis_color, start_brush):
        ...
        plot_rect = view_rect.adjusted(12.0, 24.0, -12.0, -12.0)
        ...
        painter.drawText(QRectF(plot_rect.right() - 18.0, axis_origin.y() - 10.0, 16.0, 16.0), Qt.AlignmentFlag.AlignCenter, axis_h)
        painter.drawText(QRectF(axis_origin.x() - 8.0, plot_rect.top() + 2.0, 16.0, 16.0), Qt.AlignmentFlag.AlignCenter, axis_v)
    ...
    def _draw_cnc_3d_panel_in_main_view(self, painter, view_rect, segment_groups, panel_bg, border_color, mill_pen, plunge_pen, ramp_pen, rapid_pen):
        ...
        panel_font.setPointSize(max(panel_font.pointSize() + 1, 10))
        panel_font.setBold(True)
        painter.setFont(panel_font)
        painter.setPen(QPen(QColor(215, 220, 226), 1))
        painter.drawText(view_rect.adjusted(10.0, 4.0, -10.0, 0.0), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignTop, '3D')
        ...
        preview_widget._draw_coordinate_axes_overlay(painter, plot_rect, projected, label_font=panel_font)

        stats_font = QFont(painter.font())
        stats_font.setPointSize(max(stats_font.pointSize(), 10))
        painter.setFont(stats_font)
        painter.setPen(QPen(QColor(195, 200, 208), 1))
        stats_rect = plot_rect.adjusted(10.0, 10.0, -10.0, -10.0)
        preview_x, preview_y = self._get_cnc_preview_offset_xy()
        painter.drawText(stats_rect, Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignTop, f"X {workspace[0]:.0f}  Y {workspace[1]:.0f}  Z {workspace[2]:.0f} mm")
        painter.drawText(stats_rect.adjusted(0.0, 22.0, 0.0, 0.0), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignTop, f"Offset X {preview_x:.0f}  Y {preview_y:.0f} mm")
        painter.drawText(stats_rect.adjusted(0.0, 44.0, 0.0, 0.0), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignTop, f"Move X {workspace_origin[0]:.0f}  Y {workspace_origin[1]:.0f}  Z {workspace_origin[2]:.0f} mm")
        painter.drawText(stats_rect, Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignBottom, f"P {preview_widget.rot_x:.0f}°  Y {preview_widget.rot_y:.0f}°  R {preview_widget.rot_z:.0f}°")
    ...
    def draw_main_window_cnc_toolpath_view(self, painter):
        ...
        title_rect = QRectF(rect.left() + 10.0, rect.top() + 6.0, rect.width() - 20.0, 20.0)
        painter.setPen(QPen(QColor(235, 235, 235), 1))
        painter.setFont(QFont('Arial', 20))
        ...
        content_rect = inner.adjusted(0.0, 24.0, 0.0, -2.0)
        legend_rect = QRectF(content_rect.left(), content_rect.top(), content_rect.width(), 64.0)
        views_rect = content_rect.adjusted(0.0, 68.0, 0.0, 0.0)
        ...
        painter.setBrush(QBrush(QColor(16, 18, 22, 205)))
        ...
        if self._cnc_preview_color_mode() == 'feed':
            painter.drawText(QRectF(legend_rect.left() + 10.0, legend_rect.top() + 4.0, 240.0, 16.0), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, 'Feed rate heatmap (mm/min)')

            bar_x0 = legend_rect.left() + 12.0
            bar_x1 = legend_rect.right() - 130.0
            bar_y0 = legend_rect.top() + 24.0
            bar_h = 12.0
            band_count = 10
            band_w = max(1.0, (bar_x1 - bar_x0) / float(band_count))

            for band in range(band_count):
                color = self._cnc_feed_band_color((band * 200.0) + 100.0, 245)
            ...
            for tick in range(band_count + 1):
                ...
                painter.drawLine(QPointF(x, bar_y0 + bar_h), QPointF(x, bar_y0 + bar_h + 4.0))
                label_value = tick * 200
                label = '2000+' if tick == band_count else str(label_value)
                text_rect = QRectF(x - 18.0, bar_y0 + bar_h + 5.0, 40.0, 12.0)
                painter.drawText(text_rect, Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignTop, label)

            rapid_x0 = legend_rect.right() - 108.0
            rapid_y = bar_y0 + (bar_h / 2.0)
            painter.setFont(title_font)
            painter.setPen(QPen(rapid_color, 3))
            painter.drawLine(QPointF(rapid_x0, rapid_y), QPointF(rapid_x0 + 24.0, rapid_y))
            painter.setPen(QPen(QColor(228, 232, 238), 1))
            painter.drawText(QRectF(rapid_x0 + 30.0, bar_y0 - 2.0, 70.0, 18.0), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, 'Rapid')
            painter.setFont(tick_font)
            painter.drawText(QRectF(rapid_x0 + 30.0, bar_y0 + 12.0, 86.0, 16.0), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, 'G0 travel')
        else:
            painter.drawText(QRectF(legend_rect.left() + 10.0, legend_rect.top() + 4.0, 240.0, 16.0), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, 'Motion type colors')
            legend_items = [
                ('Mill', mill_color),
                ('Plunge', plunge_color),
                ('Ramp', ramp_color),
            ...
            y = legend_rect.top() + 30.0
            for label, color in legend_items:
                painter.setPen(QPen(color, 3))
                painter.drawLine(QPointF(x, y), QPointF(x + 22.0, y))
                painter.setPen(QPen(QColor(228, 232, 238), 1))
                painter.drawText(QRectF(x + 28.0, y - 8.0, 74.0, 16.0), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, label)
                x += 108.0
    ...
    def export_stl_file(self):
        ...
        if filename:
            self.write_ascii_stl(filename, self.ski.build_ski_mesh())
